 ...
 *
 * Description:
 *    Header file defining a 3D vector class (`vec3`) with common vector operations.
 *    Used for ray tracing, graphics, and geometric computations.
 *
 * Debug Options:
...
public:
    ...
    vec3& operator/=(double t) {
    #ifdef VEC3_DEBUG
        if (t == 0) throw std::runtime_error("vec3 divide by zero");
    #endif
        return *this *= 1/t;
    }
...
#ifdef VEC3_DEBUG
    double len = v.length();
    if (len == 0) throw std::runtime_error("normalize zero-length vector");
    return v / len;
#else
    return v / v.length();
#endif
